...
nsresult nsHttpTransaction::Init(
  ...
  if (mActivityDistributor) {
    rv = mActivityDistributor->ObserveActivity(
        ...
        NS_HTTP_ACTIVITY_SUBTYPE_REQUEST_HEADER, PR_Now(), 0, headerbuf);
    if (NS_FAILED(rv)) {
  ...
  rv = NS_NewBufferedInputStream(getter_AddRefs(mRequestStream), stream.forget(),
...
SlitheenStatus nsHttpTransaction::SlitheenGetStatus()
{
